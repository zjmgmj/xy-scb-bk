...
<script>  
  ...
  export default {
    ...
    data () {
      return {
        ...
        searchFormItems: [
          ...
          {lbl: '单据号', prop: 'entryinfBillcode', val: ''},
        ...
        tableValue: {
          ...
          showRowIndex: true,
          tableHead: [
            {lbl: '单据号', prop: 'entryinfBillcode', width: '150px'},
            {lbl: '日期', prop: 'entryinfDate', width: '90px', formatter: ({cellValue}) => {
            ...
            {lbl: '业务类别', prop: 'entryinfType', formatter: ({cellValue}) => {
              const obj = { 'P001': '市场散购', 'P002': '合同入库', 'P003': '发货入库', 'P004':'装运入库' }
              return obj[cellValue]
            }}, 
            {lbl: '类型', prop: 'consignClass', width: '120px', formatter: ({cellValue}) => {
              const obj = { '0': '甲', '1': '乙', '2': '丙' }
              return obj[cellValue]
            }},
            {lbl: '采购类型', prop: 'contractCtype', width: '130px'},             
            {lbl: '结算方式', prop: 'ddTypeSettlement', width: '130px'},
            ...
            {lbl: '补差含税金额', prop: 'dataGinmoney', summary: true, g_type: 'money'},
            {lbl: '含税费用金额', prop: 'dataInfeemoney', summary: true, g_type: 'money'},
            {lbl: '付款金额', prop: 'dataFundmoney', summary: true, g_type: 'money'},
            {lbl: '补差次数', prop: 'dataSubnum'},
        ...
        tableBottomValue: {   
          ...
          tableHead: [
            ...
            {lbl: '货权转移', prop: 'entryinfDetailPermute', width: '70px', align: 'center', formatter: ({cellValue}) => {
              return cellValue === 0 ? '×' : '√'
            }},
            ...
            {lbl: '验收数量', prop: 'entryinfDetailOknum', summary: true, g_type: 'num'},
            {lbl: '验收重量', prop: 'entryinfDetailOkweight', summary: true, g_type: 'weight'},
    ...
    created () {
      this.editForm.entryinfDate = this.toDateString(new Date(), 'yyyy-MM-dd HH:mm')
    ...
    mounted () {
      this.$nextTick(()=>{
        this.queryObject = {
          page: this.currentPage,
          queryBean: {
            likeCol: ['entryinfBillcode', 'consignRemark'],
            dataChangeflag: 1,
    ...
    methods: {
      ...
      searchForm (values) {
        ...
        this.queryObject.queryBean.likeCol = ['entryinfBillcode', 'entryinfRemark']
      ...
      rowSelectionFun (row) {
        if (row.length > 0) {
          ...
          this.fileMod = {
            ...
            fileOpcode: row[0].entryinfBillcode
          }      
          const params = {
            entryinfBillcode: row[0].entryinfBillcode ? row[0].entryinfBillcode : null,
            billtypeCode: row[0].billtypeCode ? row[0].billtypeCode : null,
            feeOptcode:  row[0].entryinfBillcode ? row[0].entryinfBillcode : null,
      ...
      rowEdit (row) {
        ...
        this.editForm.entryinfBillcode = rowData.entryinfBillcode
        this.editForm.billtypeCode = rowData.billtypeCode
        this.editForm.feeOptcode = rowData.entryinfBillcode
      ...
      rowDel (row) {
        this.confirmDialog(this, '您确认要删掉记录吗？').then(() => {
          ...
          const params = {
            entryinfBillcode: list[0].entryinfBillcode,
            billtypeCode: list[0].billtypeCode,
            feeOptcode: list[0].entryinfBillcode,
      ...
      async showAudit () {
        ...
        try {
          const { data } = await this.proxy(this, 'scscm/app/entryinfAjax!showAudit.do', 'post', this.tableSelect[0])
          if (!data.needAudit) {
      ...
      async loadData () {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/entryinfAjax.do', 'post', this.queryObject)
      ...
      async delete (params) {
        try {          
          const { data } = await this.proxy(this, 'scscm/app/entryinfAjax!delete.do', 'post', params)
          console.log(data)
          if (data.map.state === '3') {
            this.dialogShow = false
      ...
      async getDetail (params) {
        try {
          const { data } = await this.proxy(this, 'scscm/app/entryinfAjax!queryList.do', 'post', params)
          console.log('*-----', data)
