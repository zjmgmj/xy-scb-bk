...
<script>  
  ...
  export default {
    ...
    data () {
      return {
        ...
        searchFormItems: [
          ...
          {lbl: '单据号', prop: 'deliveryBillcode', val: ''},
          {lbl: '提单号', prop: 'sbillBillcode', val: ''},
          ...
          {lbl: '备注', prop: 'deliveryRemark', val: ''}
        ],
        buttonGroupInfo: [
        ...
        editForm: {
          deliveryId: '',  memberCode: '', goodsBatch: '', deliveryDetailInwh: '', deliveryProperty: '', scontractMac: '', deliveryType: '', deliveryTypepro: '', ddTypeTransport: '', ddTypeSettlement: '', ddTypeDelivery: '', datasCustomer: '', datasCustomername: '', datasBalcorp: '', datasSupplier: '', datasSuppliername: '', datasAcceptcorp: '', datasAcceptcorpname: '', datasAcceptcorpaddr: '', workgroupName: '', dataTally: '', datasCarnum: '', datasDriver: '', sbillInmoney: '', sbillInfeemoney: '', feeEdittype: '', dataCondition: '', dataBackgoods: '', dataSubnum: '', dataVouchernum: '', dataVouchertype: '', dataVoucherflag: '', dataEditOrg: '', dataEditDept: '', billtypeCode: '', dataBillstate: '', dataAudit: '', dataAuditPerson: '', dataAuditRemark: '', dataAuditNremark: '', billtypeCheckmode: '', ddTypeCurrency: '', datasExrate: '', goodsNum: '', goodsPiecenum: '', goodsSingernum: '', goodsWeight: '', goodsAssistweight: '', deliveryNum: '', deliveryWeight: '', deliveryAssistweight: '', dataBnum: '', dataBweight: '', dataBassistweight: '', interfaceWmsscmCode: '', deliveryBillcode: '', deliveryDate: '', sbillBillcode: '', warehouseCode: '', warehouseName: '', deptCode: '000003', deptName: '财务部', orgCode: '000000', orgName: '测试A公司', orgAbbreviate: 'A公司', operatorName: '系统管理员', operatorCode: '0000', datasBalcorpname: '', employeeCode: '', employeeName: '', deliveryRemark: ''
        },
        tableValue: {
          hasCbx: false,
          pageination: true,
          showRowIndex: true,
          tableHead: [
            {lbl: '单据号', prop: 'deliveryBillcode', width: '150px'},
            {lbl: '日期', prop: 'deliveryDate', width: '90px', formatter: ({cellValue}) => {
              return this.toDateString(cellValue, 'yyyy-MM-dd')
            }},                             
            {lbl: '提单号', prop: 'sbillBillcode', width: '130px'},
            {lbl: '仓库', prop: 'warehouseName', width: '130px'},             
            {lbl: '结算单位', prop: 'datasBalcorpname', width: '130px'},        
            {lbl: '验收数量', prop: 'goodsNum', g_type:'num'},
            {lbl: '验收重量', prop: 'goodsWeight', g_type:'weight'},
    ...
    created () {
      this.editForm.deliveryDate = this.toDateString(new Date(), 'yyyy-MM-dd HH:mm')
    ...
    methods: {
      ...
      async showAudit () {
        ...
        try {
          const { data } = await this.proxy(this, 'scscm/app/refundBillActEnrollAjax!showAudit.do', 'post', this.tableSelect[0])
          if (!data.needAudit) {
      ...
      async loadData () {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/refundBillActEnrollAjax.do', 'post', this.queryObject)
      ...
      async delete (params) {
        try {          
          const { data } = await this.proxy(this, 'scscm/app/refundBillActEnrollAjax!delete.do', 'post', params)
          console.log(data)
          if (data.map.state === '3') {
            this.dialogShow = false
      ...
      async getDetail (params) {
        try {
          const { data } = await this.proxy(this, 'scscm/app/refundBillActEnrollAjax!queryList.do', 'post', params)
          console.log('*-----', data)
