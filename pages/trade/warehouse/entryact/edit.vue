<template lang="pug">
  el-dialog(title="入库验收登记", ref="dialog", :visible="true", fullscreen, @close="close")
    ...
    el-form.mt-10(ref="form", :model="copyForm", :rules="rules", label-width="100px")
      el-row
        el-col(:span="6")
          el-form-item(label="单据号", prop="entryactBillcode")
            el-input.full-width(v-model="copyForm.entryactBillcode", clearable, size="mini", readonly)
        el-col(:span="6")
          el-form-item(label="日期", prop="entryactDate")
            el-date-picker.full-width(v-model="copyForm.entryactDate", type="datetime", format="yyyy-MM-dd HH:mm", value-format="yyyy-MM-dd HH:mm", clearable, size="mini")
        el-col(:span="6")
...
export default {
  ...
  data () {
    return {
      ...
      detailTable: {      
        id: 'entryActUp',
        ref: 'detailTable',
        relationBatch: 'entryactBillbatch', // TODO 过滤明细和码单
        loading: false,
        buttonGroupInfo: [],
        tableValue: {
          hasCbx: false,
          ...
          tableHead: [
            ...
            {lbl: '验收数量', prop: 'goodsNum', summary: true, g_type: 'num', noedit: true},
            {lbl: '验收重量', prop: 'goodsWeight', summary: true, g_type: 'weight', noedit: true},
            ...
            {lbl: '发货日期', prop: 'consignDetailDate', width: '150px', noedit: true, formatter: ({cellValue}) => {
            ...
            {lbl: '备注', prop: 'contractDetailRemark', noedit: true}
          ],
          hiddenProp: {
  ...
  created () {
    let isTrue = false
    if (!this.form.entryactBillcode) isTrue = true
    this.topButtonGroupInfo = [
      {lbl: '保存', type: 'save', icon: 'el-icon-folder-checked'},      
      {lbl: '入库通知', type: 'entryinf', icon: 'el-icon-document'},
  ...
  beforeMount () {
    ...
    if (this.form.entryactBillcode) {
      this.fileMod = {
        fileBilltype: this.copyForm.billtypeCode,
        fileOpcode: this.copyForm.entryactBillcode
      }
      this.queryParams = {
        entryactBillcode: this.copyForm.entryactBillcode,
        billtypeCode:this.copyForm.billtypeCode,
        feeOptcode: this.copyForm.entryactBillcode,
  ...
  methods: {
    ...
    insertTableData ({data, mainMod, tableMod, itemFun}) {
      ...
      resData.map((item, index) => {
        ...
        if (item.interfaceWmsscmEflag === 1 && item.interfaceWmsscmCode) {
          entryinfBillcodeArr.push(item.entryinfBillcode)
          entryinfBillbatchArr.push(item.goodsEntrybatch)
        }
        // ----
      ...
      this.fileMod = {
        ...
        fileOpcode: data[0].entryactBillcode
      }
    ...
    codeBtnClick(flag) {
      switch(flag) {        
        case 'add': // 新增码单    
          ...
          insertData.entryactBillcode = detailSelectRow.entryactBillcode
    ...
    async saveAsync (params) {
      ...
      try {
        ...
        if (this.copyForm.entryactBillcode) {
