  ...
  export default {
    ...
    data () {
      return {
        ...
        searchFormItems: [
          ...
          {lbl: '审核', prop: 'dataAudit', val: '', type: 'select', list: []},
    ...
    mounted () {
      this.$nextTick(()=>{
        this.queryObject = {
          page: this.currentPage,
          queryBean: {
            likeCol: ['scontractBillcode', 'scontractRemark'],
            dataChangeflag: 0,
    ...
    methods: {
      ...
      searchForm (values) {
        ...
        this.queryObject.queryBean.likeCol = ['scontractBillcode', 'scontractRemark']
        this.queryObject.queryBean.dataChangeflag = 0
        this.currentPage = 1
      ...
      async showAudit () {
        ...
        try {
          const { data } = await this.proxy(this, 'scscm/app/scontractEnrollAjax!showAudit.do', 'post', this.tableSelect[0])
          if (!data.needAudit) {
      ...
      async loadData () {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/scontractEnrollAjax.do', 'post', this.queryObject)
      ...
      async delete (params) {
        try {          
          const { data } = await this.proxy(this, 'scscm/app/scontractEnrollAjax!delete.do', 'post', params)
          console.log(data)
          if (data.map.state === '3') {
            this.dialogShow = false
      ...
      async getDetail (params) {
        try {
          const { data } = await this.proxy(this, 'scscm/app/scontractEnrollAjax!queryDetail.do', 'post', params)
          console.log('*-----', data)
