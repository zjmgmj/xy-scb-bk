...
<script>  
  ...
  export default {
    ...
    data () {
      return {        
        collapseName: ['1'],
        treeData: [{id: '0', text: '单位控制', filterObj:"'{creditGrade:'0'}'", children: [
          {text: '机构控制', id:'1', filterObj:"'{creditGrade:'1'}'"}
        ]}],
        treeProps: {
          children: 'children',
          label: 'text',
    ...
    methods: {
      ...
      async loadData () {
        this.loading = true
        try {          
          const { data } = await this.proxy(this, 'scscm/app/saleCreditAjax.do', 'post', this.queryObject)
      ...
      async delete (params) {
        try {          
          const { data } = await this.proxy(this, 'scscm/app/saleCreditAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
