<template lang="pug">
  el-dialog(title="销售合同变更", ref="dialog", :visible="true", fullscreen, @close="close")
    ...
    el-form.mt-10(ref="form", :model="copyForm", :rules="rules", label-width="100px")
      el-row
        ...
        el-col(:span="6")
          el-form-item(label="原合同号", prop="scontractBillcodeold")
            el-input.full-width(v-model="copyForm.scontractBillcodeold", clearable, size="mini", readonly)    
    .mt-10.edit
    ...
    //- 销售合同
    scontract-enroll-Win(v-if="scontractWinShow", :uniqueField="'scontractBillbatch'", :parentRef="table.$refs.detailTable.$refs.basicEditable", :queryParam="scontractQueryParam", @close="scontractWinShow=false", @cellBack="scontractCellBack")
    //- 特权密码
    password-win(v-if="passwordShow", :config="pWConfig", @cellback="pwCellBack", @close="passwordShow = false")
    //- password-win(v-if="passwordShow", :config="pWConfig")
</template>
<script>
...
export default {
      ...
      detailConfig: {
        ...
        tableValue: {        
          ...
          tableHead: [
            ...
            {lbl: '重量', prop: 'goodsWeight', summary: true, g_type: 'weight', minValue:0, change: ({cellValue, row}) => {         
              this.pricemoneycomputer('grid', row, 'goodsWeight', this.table.$refs.detailTable.priceMoneyComputercolumn)
              this.table.$refs.detailTable.$refs.basicEditable.updateFooter()
            }},
            ...
            {lbl: '含税金额1', prop: 'goodsInmoney', summary: true, g_type: 'money'},            
            {lbl: '无税单价', prop: 'goodsExprice', g_type:'exprice'},
            {lbl: '无税金额1', prop: 'goodsExmoney', summary: true, width: '120px', g_type: 'money'},
    ...
    switch (this.flag) {
      case 0:
        this.topButtonGroupInfo = [
          ...
          {lbl: '销售合同', type: 'scontract', icon: 'el-icon-document'},
  ...
  methods: {
    ...
    sumGoodsCellBack ({data}) {
      ...
      const tableMod = {scontractDetailPartsname: 'partsnameName', scontractDetailMaterial: 'goodsMaterial', scontractDetailSpec: 'goodsSpec', scontractDetailProareaname: 'productareaName', scontractDetailSpec1: 'goodsSpec1', scontractDetailSpec2: 'goodsSpec2', scontractDetailSpec3: 'goodsSpec3', scontractDetailSpec4: 'goodsSpec4', scontractDetailProperty1: 'goodsProperty1', scontractDetailProperty2: 'goodsProperty2', scontractDetailProperty3: 'goodsProperty3', scontractDetailProperty4: 'goodsProperty4', scontractDetailProperty5: 'goodsProperty5', scontractDetailNumproperty1: 'goodsNumproperty1', warehouseName: 'warehouseName', warehouseCode: 'warehouseCode', goodsOrg: 'goodsOrg', goodsOrgName: 'goodsOrgName', goodsDept: 'goodsDept', goodsDeptName: 'goodsDeptName', goodsNum: 'goodsSupplynum', goodsWeight: 'goodsSupplyweight', goodsAssistweight: 'goodsSupplyassistwieght', goodsInprice: 'goodsNewinprice', goodsInfeeprice: 'goodsNewinfeeprice', goodsNewinprice: 'goodsNewinprice', goodsNewinfeeprice: 'goodsNewinfeeprice', goodsCosttaxrate: 'goodsTaxrate', goodsBinprice: 'pricesetMakeprice', goodsInmoney: 'goodsSupplyInMoney', goodsExprice: 'pricesetMakeExprice', goodsExmoney: 'goodsSupplyExMoney', goodsTaxrate: 'goodsTaxrate', goodsTaxmoney: 'goodsSupplyTrMoney'}
    ...
    sdpGoodsCellBack ({data}) {    
      ...
      if (this.codeFutures) {
        ...
        if (tableDetail.length > 0) this.table.detailInsertAt(tableDetail)
    ...
    scontractCellBack ({data}) {
      // 销售合同      
      const scontractBillcodeList = []
      const scontractBillbatchList = []
      debugger
      Array.from(data).map(item => {
        scontractBillcodeList.push(item.scontractBillcode)
        scontractBillbatchList.push(item.scontractBillbatch)
      })
      const scontractBillcodeStr = scontractBillcodeList.toString()
      const scontractBillbatchStr = scontractBillbatchList.toString()
      const codeBean = {
        scontractBillcodeStr: scontractBillcodeStr,
        scontractBillbatchStr: scontractBillbatchStr
      }
      const scontractBillcode = data[0].scontractBillcode
      const billtypeCode = data[0].billtypeCode
      this.getMainMod({scontractBillcode: scontractBillcode}, 'scontract')
      this.getTermDetail({termlistOptcode: scontractBillcode, billtypeCode: billtypeCode})
      this.getImpCodeList({codeBean: codeBean})
      // scontractBillbatch
      ...
      switch (type) {
        ...
        case 'scontract':
          ...
          this.scontractQueryParam = {
            ...
            orgCode: this.copyForm.orgCode,
            deptCode: this.copyForm.deptCode,
            scontractDetailType: '',
            scontractDetailMac: 0,
            scontractBillcode: this.copyForm.scontractBillcodeold,
            report: false,
            goodsFlag: false
          }
    ...
    async getMainMod (params, flag) {
      try {
        const { data } = await this.proxy(this, 'scscm/app/schangeEnrollAjax!edit.do', 'post', params)        
        if (data.map.state !== '3') {
          // this.btnFlag = ''
        ...
        if (flag === 'scontract') {
          const mainProp = {orgName:'orgName',deptName:'deptName',employeeName:'employeeName',dataAuditStr:'dataAuditStr',scontractClassStr:'scontractClassStr',memberCode:'memberCode',scontractOthercode:'scontractOthercode',goodsBatch:'goodsBatch',scontractType:'scontractType',scontractTypepro:'scontractTypepro',scontractMac:'scontractMac',scontractCtype:'scontractCtype',scontractClass:'scontractClass',datasCustomer:'datasCustomer',datasCustomername:'datasCustomername',datasBalcorp:'datasBalcorp',datasBalcorpname:'datasBalcorpname',companyProjectCode:'companyProjectCode',companyProjectName:'companyProjectName',datasAcceptcorp:'datasAcceptcorp',datasAcceptcorpname:'datasAcceptcorpname',datasAcceptcorpaddr:'datasAcceptcorpaddr',feeEdittype:'feeEdittype',scontractRemark:'scontractRemark',orgCode:'orgCode',deptCode:'deptCode',employeeCode:'employeeCode',ddTypeCurrency:'ddTypeCurrency',datasExrate:'datasExrate',goodsPiecenum:'goodsPiecenum',goodsSingernum:'goodsSingernum',scontractBillcodeold:'scontractBillcode'}
          Object.keys(mainProp).forEach((key) => {            
            this.copyForm[key] = data.mainMod[mainProp[key]] === null ? '' : data.mainMod[mainProp[key]]
          })
          return false          
        }
        console.log(data)        
    ...
    async getDetail () {
      try {
        this.loading = true
        const { data } = await this.proxy(this, 'scscm/app/schangeEnrollAjax!queryDetail.do', 'post', this.queryParams)
        console.log(data)
    ...
    async getCodeList () {
      try {
        this.loading = true
        const { data } = await this.proxy(this, 'scscm/app/schangeEnrollAjax!queryCode.do', 'post', this.queryParams)
        console.log(data)
    ...
    async getImpCodeList (params) {
      try {
        this.loading = true
        const { data } = await this.proxy(this, 'scscm/app/schangeEnrollAjax!queryImpCodeList.do', 'post', params)
        console.log(data)
        debugger
        const detailTable = this.table.$refs.detailTable.$refs.basicEditable
        const tableDetail = []
        if (data.reportList !== null && data.reportList.length > 0) {
          data.reportList.map(item => {
            // 明细赋值
            item.dataFlag = '0'
            item.scontractDetailOptcode = item.scontractBillcode
            item.scontractDetailOptbatch = item.scontractBillbatch
            item.scontractDetailBilltype = item.billtypeCode
            item.scontractBillcodeo = item.scontractBillcode
            item.scontractBillbatcho = item.scontractBillbatch
            item.scontractBillcode = ''
            if (item.scontractDetailCode !== 1) {
              item.goodsNum = item.scontractDetailNsbnum
              item.goodsWeight = item.scontractDetailNsbweight
              item.goodsAssistweight = item.scontractDetailNsbassistwieght
            } else {
              item.goodsNum = 0
              item.goodsWeight = 0
              item.goodsAssistweight = 0
            }
            item.goodsInmoney = item.goodsBinprice * item.goodsWeight
            item.goodsExmoney = item.goodsExprice * item.goodsWeight
            item.goodsTaxmoney = item.goodsInmoney - item.goodsExmoney
            item.scontractDetailDassistweight = ''
            item.dataInvoiceflag = ''
            item.scontractDetailMweight = ''
            item.scontractDetailCweight = ''
            item.scontractDetailDnum = ''
            item.contractDetailChassistweight = ''
            item.scontractDetailFassistweight = ''
            item.scontractDetailDweight = ''
            item.scontractDetailChweight = ''
            item.scontractDetailBweight = ''
            item.scontractDetailState = ''
            item.dataInvweight = ''
            item.scontractDetailEweight = ''
            item.dataGoodsok = ''
            item.scontractDetailCnum = ''
            item.scontractDetailBnum = ''
            item.dataVernum = ''
            item.dataOtherflag = ''
            item.scontractDetailBassistweight = ''
            item.dataInvnum = ''
            item.dataInvmoney = ''
            item.scontractDetailMassistweight = ''
            item.scontractDetailEnum = ''
            item.dataGoodsflag = ''
            item.scontractDetailFweight = ''
            item.scontractDetailEassistweight = ''
            item.scontractDetailOnum = ''
            item.scontractDetailOassistweight = ''
            item.dataFundmoney = ''
            item.scontractDetailOweight = ''
            item.scontractDetailFnum = ''
            item.scontractDetailCassistweight = ''
            item.scontractDetailMnum = ''
            item.scontractDetailChnum = ''
            item.scontractDetailId = ''
            item.dataFundflag = ''
        ...
        if (data.codeList !== null && data.codeList.length > 0) {
          ...
          data.codeList.map(item => {
            // 码单赋值
            item.dataFlag = "0"
            item.scontractBillcodeold = item.scontractBillcode
            item.scontractBillbatchold = item.scontractBillbatch
            item.scontractCodeBatchold = item.scontractCodeBatch
            item.scontractBillcode = ""
            item.scontractCodeBatch = ""
            item.goodsNum = item.scontractCodeNBnum
            item.goodsAssistweight = item.scontractCodeNBassistweight
            item.goodsWeight = item.scontractCodeNBweight
            item.scontractCodeMweight = ''
            item.scontractCodeOnum = ''
            item.scontractCodeMassistweight = ''
            item.scontractCodeChweight = ''
            item.scontractCodeDweight = ''
            item.scontractCodeNum = ''
            item.dataVernum = ''
            item.scontractCodeState = ''
            item.scontractCodeDassistweight = ''
            item.scontractCodeWeight = ''
            item.scontractCodeOweight = ''
            item.scontractCodeId = ''
            item.scontractCodeChnum = ''
            item.scontractCodeChassistweight = ''
            item.scontractCodeDnum = ''
            item.scontractCodeOassistweight = ''
            item.scontractCodeMnum = ''
            codeList.push(item)
          })
          this.table.codeInsertAt(codeList, true) // 插入码单     
        }
        if (data.mtaskReportList !== null && data.mtaskReportList.length > 0) {
          const tableList = this.$refs.sdpMac.tableList
          data.mtaskReportList.map(item => {
            item.dataFlag = '0'
            tableList.push(item)
          })
        }
    ...
    async checkSaleNum (params) {      
      const loading = this.loadShow()
      try {
        const api = 'scscm/app/schangeEnrollAjax!checkSaleNum.do'
        const { data } = await this.proxy(this, api, 'post', {scontractStr: JSON.stringify(params), editType: 0})
        console.log('checkSaleNum', data)
        loading.close()
    ...
    async saveAsync (params, loading) {
      // 保存      
      try {
        let api = 'scscm/app/schangeEnrollAjax!insert.do'
        if (this.flag === 1) {
          api = 'scscm/app/schangeEnrollAjax!update.do'
        }
