...
<script>  
  ...
  import editBox from '@/pages/trade/warehouse/entryinf/edit'
  ...
  export default {
    ...
    data () {
      return {
        ...
        searchFormItems: [
          ...
          {lbl: '单据类型', prop: 'billtypeCode', val: '', type: 'billtype', typeStr:'W002,W026'},
        ...
        tableValue: {   
          ...
          tableHead: [
            ...
            {lbl: '收货机构', prop: 'datasInorgName', width: '140px'},
            {lbl: '仓库', prop: 'warehouseName', width: '140px'},
            {lbl: '供货单位', prop: 'datasSuppliername', width: '140px'},
            {lbl: '结算单位', prop: 'datasBalcorpname', width: '140px'},
            {lbl: '业务类别', prop: 'entryinfTypeStr', width: '140px'},            
            {lbl: '类型', prop: 'entryinfClassStr', width: '50px', align: 'center'},
            {lbl: '采购类型', prop: 'contractCtype', width: '140px'},    
            {lbl: '结算方式', prop: 'ddTypeSettlement', width: '140px'},  
            ...
            {lbl: '销售合同号', prop: 'scontractBillcode', width: '150px'},
            {lbl: '业务单号', prop: 'entryinfDetailOptcode', width: '150px'}
          ],
          tableData: []
        },
        currentPage: 1,
    ...
    methods: {    
      tableCellClick ({row, column}) {        
        if (column.title === '单据号') {
          this.editForm = {
            entryinfBillcode: row.entryinfBillcode,
            billtypeCode: row.billtypeCode,
            feeOptcode: row.entryinfBillcode,
      ...
      consignSure (flagMsg, type) {
        ...
        this.tableSelect.map(item => {
          ...
          completeCodeArr.push(item.entryinfBillcode)
          completeBatchArr.push(item.goodsEntrybatch)
      ...
      async loadData () {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/entryinfAjax!queryReport.do', 'post', this.queryObject)
      ...
      async saveAsync (param, flag, type) {
        ...
        try {
          ...
          let params = {}
          if (type === 'goods') {
            api = 'scscm/app/entryinfAjax!updateGoodsComplete.do'                                
            if (!flag) {
              api = 'scscm/app/entryinfAjax!updateGoodsUnComplete.do'
            }
            params = {detailBean: param}
          } else if (type === 'invoice') {
            api = 'scscm/app/entryinfAjax!updateInvoiceComplete.do'
            if (!flag) {
              api = 'scscm/app/entryinfAjax!updateInvoiceUnComplete.do'
            }
            params = {opBean: param}
          }
