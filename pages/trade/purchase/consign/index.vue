...
<script>  
  ...
  export default {
      ...
      return {
        ...
        tableBottomValue: {   
          ...
          tableHead: [
            ...
            {lbl: '数量', prop: 'planDetailAnum', summary: true, g_type: 'num'},
            {lbl: '重量', prop: 'planDetailAweight', summary: true, g_type: 'weight'},
    ...
    created () {
      this.editForm.consignDate = this.toDateString(new Date(), 'yyyy-MM-dd HH:mm')
    ...
    mounted () {
      this.$nextTick(()=>{
        this.queryObject = {
          page: this.currentPage,
          queryBean: {
            likeCol: ['consignBillcode', 'consignRemark'],
            dataChangeflag: 1,
    ...
    methods: {
      ...
      searchForm (values) {
        ...
        this.queryObject.queryBean.likeCol = ['consignBillcode', 'consignRemark']
      ...
      rowSelectionFun (row) {
        if (row.length > 0) {
          ...
          this.fileMod = {
            ...
            fileOpcode: row[0].consignBillcode
          }      
          const params = {
            consignBillcode: row[0].consignBillcode ? row[0].consignBillcode : null,
            billtypeCode: row[0].billtypeCode ? row[0].billtypeCode : null,
            feeOptcode:  row[0].consignBillcode ? row[0].consignBillcode : null,
      ...
      rowEdit (row) {
        ...
        this.editForm.consignBillcode = rowData.consignBillcode
        this.editForm.billtypeCode = rowData.billtypeCode
        this.editForm.feeOptcode = rowData.consignBillcode
      ...
      rowDel (row) {
        this.confirmDialog(this, '您确认要删掉记录吗？').then(() => {
          ...
          const params = {
            consignBillcode: list[0].consignBillcode,
            billtypeCode: list[0].billtypeCode,
            feeOptcode: list[0].consignBillcode,
      ...
      async showAudit () {
        ...
        try {
          const { data } = await this.proxy(this, 'scscm/app/purchaseConsignAjax!showAudit.do', 'post', this.tableSelect[0])
          if (!data.needAudit) {
      ...
      async loadData () {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/purchaseConsignAjax.do', 'post', this.queryObject)
      ...
      async delete (params) {
        try {          
          const { data } = await this.proxy(this, 'scscm/app/purchaseConsignAjax!delete.do', 'post', params)
          console.log(data)
          if (data.map.state === '3') {
            this.dialogShow = false
      ...
      async getDetail (params) {
        try {
          const { data } = await this.proxy(this, 'scscm/app/purchaseConsignAjax!queryList.do', 'post', params)
          console.log('*-----', data)
