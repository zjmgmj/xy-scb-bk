  ...
  export default {
    ...
    data() {
      return {
        collapseName: ['1'],
        searchFormItems: [
          { lbl: '代码', prop: 'applicationCode', val: '' },
          { lbl: '名称', prop: 'applicationName', val: '' }
        ],
        buttonGroupInfo: [
        ...
        tableValue: {
          ...
          showRowIndex: true,
          tableHead: [
            { lbl: '代码', prop: 'applicationCode' },
            { lbl: '名称', prop: 'applicationName' },
            {
              lbl: '状态', prop: 'applicationState', formatter: ({ cellValue }) => {
                const o = { 0: '√', 1: '×' }
                return o[cellValue] || cellValue
              }
            },
            { lbl: '顺序', prop: 'applicationOrder' }
          ],
          tableData: []
        },
        currentPage: 1,
        ...
        queryObject: {
          queryBean: {
            likeCol: ['applicationCode', 'applicationName']
          },
          otherlikeCol: 0,
          sort: '[{"property":"applicationOrder","direction":"ASC"}]'
        },
        loading: false,
        dialogShow: false,
        dialogTitle: '系统应用',
    ...
    methods: {
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/applicationAjax.do', 'post', this.queryObject)
          if ('applicationList' in data) {
            this.tableValue.tableData = data.applicationList
