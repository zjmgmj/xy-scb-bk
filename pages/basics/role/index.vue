<template lang="pug">
  erplr-panel(:right-padding="false")
    ...
    .erp-content(slot="right")
      ...
      el-dialog(ref="dialog", :title="dialogTitle", :visible="dialogShow",  width="350px", @close="editCancel")
  ...
  export default {
    ...
    data() {
      return {
        collapseName: ['1'],
        searchFormItems: [
          { lbl: '角色代码', prop: 'roleCode', val: '', placeholder: '请输入姓名' },
          { lbl: '角色名称', prop: 'roleName', val: '', placeholder: '请输入代码' },
          { lbl: '备注', prop: 'roleRemark', val: '', placeholder: '请输入代码' }
        ],
        buttonGroupInfo: [
        ...
        tableValue: {
          ...
          showRowIndex: true,
          tableHead: [
            { lbl: '角色代码', prop: 'roleCode', width: '100px' },
            { lbl: '角色名称', prop: 'roleName', width: '100px' },
            { lbl: '备注', prop: 'roleRemark' },
            {
        ...
        queryObject: {
          queryBean: {
            likeCol: ['roleCode', 'roleName', 'roleRemark']
          },
          otherlikeCol: 0,
          sort: '[{"property":"roleCode","direction":"DESC"}]'
        },
        loading: false,
        dialogShow: false,
        dialogTitle: '角色设置',
    ...
    methods: {
      ...
      rowDel(row) {
        this.confirmDialog(this, '您确认要删掉记录吗？').then(() => {
          ...
          const params = {
            ...
            roleCode: list[0].roleCode
          }
          this.delete({ role: params })
        })
      },
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/roleAjax.do', 'post', this.queryObject)
          if ('roleList' in data) {
            this.tableValue.tableData = data.roleList
      ...
      async delete(params) {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/roleAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
