<template lang="pug">
  erplr-panel(:right-padding="false")
    ...
    .erp-content(slot="right")
      ...
      excel-import(
        upload-url="scscm/app/storageGoodsInitAjax!uploadInitgoods.do",
        import-url="scscm/app/storageGoodsInitAjax!importInitgoods.do",
        :extraParam="upload",
  ...
  export default {
    ...
    data() {
      return {
        collapseName: ['1'],
        searchFormItems: [
          { lbl: '货主', prop: 'companyCode', val: '', type: 'company', typeStr: 'HZ' },
        ...
        tableValue: {
          ...
          tableHead: [
            ...
            { lbl: '数量单位', prop: 'partsnameNumunit', width: '120px' },
            { lbl: '重量单位', prop: 'partsnameWeightunit', width: '120px' },
            { lbl: '计量方式', prop: 'goodsMetering', width: '120px' },
    ...
    methods: {
      ...
      downTemplate() {
        const url = 'scscm/app/storageGoodsInitAjax!downTemplet.do'
        const excelBean = {}
        excelBean.headerName = this.tableValue.tableHead.map(x => x.lbl)
        excelBean.valueName = this.tableValue.tableHead.map(x => x.prop)
        excelBean.valueFormatMapStr = 'goodsNum;###,###>goodsWeight;###,###.0000'
        excelBean.sheetName = '初始化货主物资码单导入'
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/storageGoodsInitAjax.do', 'post', this.queryObject)
          if ('list' in data) {
      ...
      async delete(params) {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/storageGoodsInitAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
