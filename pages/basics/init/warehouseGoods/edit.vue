<template lang="pug">
  el-form(ref="dialogForm", :model="form", :rules="rules", label-width="90px")
    ...
    .row
      ...
      .col
        el-form-item(label="商品属性", prop="goodsCodestr6")
          el-input(v-model="form.goodsCodestr6", clearable, size="mini")
    .row
      .col
    ...
    .row
      .col
        el-form-item(label="税率", prop="goodsTaxrate")
          el-input.full-width(v-model="form.goodsTaxrate", clearable, size="mini", @change="formatNum($event, '+2', 'form.goodsTaxrate')")
      .col
        el-form-item(label="单价", prop="goodsInprice")
          el-input.full-width(v-model.number="form.goodsInprice", clearable, size="mini", @change="formatNum($event, '+2', 'form.goodsInprice')")
      .col
        el-form-item(label="费用单价", prop="goodsInfeeprice")
          el-input.full-width(v-model.number="form.goodsInfeeprice", clearable, size="mini", @change="formatNum($event, '+2', 'form.goodsInfeeprice')")
    .row
      .col
      ...
      .col
        el-form-item(label="件支数", prop="goodsPartsbranch")
          el-input.full-width(v-model.number="form.goodsPartsbranch", clearable, size="mini")
      .col
        el-form-item(label="卡号", prop="goodsContractstr3")
          el-input.full-width(v-model="form.goodsContractstr3", clearable, size="mini")
    .row
      .col
        el-form-item(label="总车数", prop="goodsContractstr5")
          el-input.full-width(v-model="form.goodsContractstr5", clearable, size="mini")
      .col
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async asyncSave() {
        try {
          let api = 'scscm/app/warehouseGoodsInitAjax!save.do'
          if (this.form.goodsCodebill) {
            api = 'scscm/app/warehouseGoodsInitAjax!update.do'
          }
