<template lang="pug">
  erplr-panel(:right-padding="false")
    ...
    .erp-content(slot="right")
      ...
      excel-import(
        upload-url="scscm/app/invoicePurchaseInitAjax!uploadInitgoods.do",
        import-url="scscm/app/invoicePurchaseInitAjax!importInitgoods.do",
        :extraParam="upload",
...
<script>
  ...
  export default {
    layout: 'backend',
    name: 'BasicsInitInitinvoicePurchase',
    components: {
      basicElxTable,
      erplrPanel,
      buttonGroup,
    ...
    methods: {
      ...
      downTemplate() {
        const url = 'scscm/app/invoicePurchaseInitAjax!downTemplet.do'
        const excelBean = {}
        ...
        excelBean.sheetName = '初始化采购未到票导入'
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/invoicePurchaseInitAjax.do', 'post', this.queryObject)
          if ('list' in data) {
      ...
      async delete(params) {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/invoicePurchaseInitAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
