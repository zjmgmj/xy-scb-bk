<template lang="pug">
  erplr-panel(:right-padding="false")
    ...
    .erp-content(slot="right")
      ...
      excel-import(
        upload-url="scscm/app/invoiceFeeinInitAjax!uploadInitgoods.do",
        import-url="scscm/app/invoiceFeeinInitAjax!importInitgoods.do",
        :extraParam="upload",
...
<script>
  ...
  export default {
    layout: 'backend',
    name: 'BasicsInitInitinvoiceFeein',
    components: {
      basicElxTable,
      erplrPanel,
      buttonGroup,
    ...
    methods: {
      ...
      downTemplate() {
        const url = 'scscm/app/invoiceFeeinInitAjax!downTemplet.do'
        const excelBean = {}
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/invoiceFeeinInitAjax.do', 'post', this.queryObject)
          if ('list' in data) {
      ...
      async delete(params) {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/invoiceFeeinInitAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
