<template lang="pug">
  erplr-panel(:right-padding="false")
    ...
    .erp-content(slot="right")
      ...
      excel-import(
        upload-url="scscm/app/invoiceSaleInitAjax!uploadInitgoods.do",
        import-url="scscm/app/invoiceSaleInitAjax!importInitgoods.do",
        :extraParam="upload",
...
<script>
  ...
  export default {
    layout: 'backend',
    name: 'BasicsInitInitinvoiceSale',
    components: {
      basicElxTable,
      erplrPanel,
      buttonGroup,
    ...
    methods: {
      ...
      downTemplate() {
        const url = 'scscm/app/invoiceSaleInitAjax!downTemplet.do'
        const excelBean = {}
        ...
        excelBean.sheetName = '初始化销售未开票导入'
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/invoiceSaleInitAjax.do', 'post', this.queryObject)
          if ('list' in data) {
      ...
      async delete(params) {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/invoiceSaleInitAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
