<template lang="pug">
  erplr-panel(:right-padding="false")
    ...
    .erp-content(slot="right")
      ...
      excel-import(
        upload-url="scscm/app/saleInitAjax!uploadInitgoods.do",
        import-url="scscm/app/saleInitAjax!importInitgoods.do",
        :extraParam="upload",
...
<script>
  ...
  export default {
    layout: 'backend',
    name: 'BasicsInitInitaccountglideSale',
    components: {
      basicElxTable,
      erplrPanel,
      buttonGroup,
    ...
    methods: {
      ...
      downTemplate() {
        const url = 'scscm/app/saleInitAjax!downTemplet.do'
        const excelBean = {}
        ...
        excelBean.sheetName = '初始化销售往来金额导入'
      ...
      async loadData() {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/saleInitAjax.do', 'post', this.queryObject)
          if ('list' in data) {
      ...
      async delete(params) {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/saleInitAjax!delete.do', 'post', params)
          if (data.map.state === '3') {
            this.dialogShow = false
