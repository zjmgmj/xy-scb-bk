...
erplr-panel(:right-padding="false")
  div(slot="left")
    el-collapse(accordion, v-model="collapseName")
      el-collapse-item.slot-left(title="仓库列表", name="1")
        //- el-tree(ref="leftTree", :data="treeData", :props="treeProps", @nodeClick="treeNodeClick")
  ...
  .erp-content(slot="right")
    ...
    el-dialog(ref="dialog", title="库区设置", :visible="dialogShow", width="800px", @close="editCancel")
...
<script>  
  ...
  export default {
    ...
    data () {
      return {
        collapseName: ['1', '2'],
        searchFormItems: [{lbl: '库区', prop: 'wareareaName', val: ''}],
        leftOrgDialog: false,
        leftOrgTitle: '新增组织',
        ...
        tableValue: {          
          ...
          tableHead: [
            ...
            {lbl: '库区', prop: 'wareareaName', width: '100px'},
            {lbl: '货区', prop: 'wareareaGregion'},             
            {lbl: '最低存放量', prop: 'wareareaMinnum', width: '120px'},
            {lbl: '最大存放量', prop: 'wareareaMaxnum', width: '120px'},
            {lbl: '图形行', prop: 'wareareaX'},
            {lbl: '图形列', prop: 'wareareaY'},
            {lbl: '高', prop: 'wareareaHeight'},
            {lbl: '宽', prop: 'wareareaWidth'},
            {lbl: '备注', prop: 'wareareaRemark'},
            {lbl: '类型', prop: 'wareareaCtype'}, 
        ...
        queryObject: {
          ...
          'queryBean.likeCol': ['wareareaName'],  
          otherlikeCol:0,
          start: 0,
          limit: this.pageSize
        },
    ...
    methods: {
      ...
      searchForm (values) {
        ...
        this.queryObject['queryBean.likeCol'] = ['wareareaName']
      ...
      rowDel (row) {
        this.confirmDialog(this, '您确认要删掉记录吗？').then(() => {
          ...
          const params = {
            wareareaCode: list[0].wareareaCode
          }
          this.delete(this.paramsFormat('warearea', params))
        })
      },
      ...
      async loadData () {
        this.loading = true
        try {
          const { data } = await this.proxy(this, 'scscm/app/wareareaAjax.do', 'post', this.queryObject)
          this.tableValue.tableData = []
          this.totalCount = 0
          if (data.wareareaList.length > 0) {
            this.tableValue.tableData = data.wareareaList
      ...
      async delete (params) {
        try {          
          const { data } = await this.proxy(this, 'scscm/app/wareareaAjax!delete.do', 'post', params)
          console.log(data)
          if (data.map.state === '3') {
            this.dialogShow = false
